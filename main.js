...
const getPokemonData = (pokemon) => {
  fetch(`https://pokeapi.co/api/v2/pokemon/${pokemon}`)
    ...
    .then((data) => {
      ...
      typeScreen.innerHTML = `Height: ${data.height * 10}cm ` ;
      idScreen.innerHTML = `Weight: ${data.weight / 10}kg` ;
      aboutScreen.innerHTML = `Type: ${data.types[0].type.name} <br> id #: ${data.id}`;
